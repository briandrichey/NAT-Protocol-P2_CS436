...
def process_pkt_private(pkt: Packet):  
    ...
    if((srcIp != ignoreIp ) and (dstIp != ignoreIp) ):#and (srcIp != routPubIP)
        if ((pkt.sniffed_on == PRIVATE_IFACE) and (srcIp == routPubIP)):
            print("received private pkt", pkt.sniffed_on, pkt.summary())
            return
        if ((pkt.sniffed_on == PRIVATE_IFACE) and (srcIp != routPubIP)):
            print("received private pkt", pkt.sniffed_on, pkt.summary())
    
            if (IP in pkt and IP not in pvtIpMap):#ip not mapped
                srcIp = pkt[IP].src  # gets source and destination ip/// ie. '0.0.0.0'
                dstIp = pkt[IP].dst
            ...
            if (TCP in pkt and IP not in clientTcpIpPortMap ):#TCP not mapped then create port connection and send
                ...
                serverTcpIpPortMap[str(routPubIP)] = servernewPort #records routers public port number via srcIP key in dictionary
                
                if(srcIp != pubIpMap[routPubIP]):#src == client outgoing pub pkt 
                    TcpDst = pvtIpMap[str(srcIp)]
                    clientnewPort = randint(2250, 2500) #chooses port in 2250-2500 range
                    clientTcpIpPortMap[str(srcIp)] = clientnewPort

                    newPubTcpPkt = IP(src = routPubIP , dst = TcpDst)/ TCP(sport=serverTcpIpPortMap[str(routPubIP)], dport = serverPort)
                    #pkt[TCP] #uses separate tables to gather address info
                    #newPubTcpPkt[TCP].sport = serverTcpIpPortMap[str(routPubIP)]
...
def process_pkt_public(pkt: Packet):
    ...
    srcIp = pkt[IP].src  # gets source and destination ip/// ie. '0.0.0.0'
    dstIp = pkt[IP].dst
    if((srcIp != ignoreIp ) and (dstIp != ignoreIp)):#
        if ((pkt.sniffed_on == PUBLIC_IFACE) and (srcIp == routPubIP)):
        ...
        if ((pkt.sniffed_on == PUBLIC_IFACE) and (srcIp != routPubIP)):
            ...
            if (TCP in pkt and IP not in serverTcpIpPortMap):
                ...
                if (srcIp == pubIpMap[str(routPubIP)]):#src == server
                    ...
                    newPvtTcpPkt = IP(src = srcIp , dst = TcpDst)/ TCP(dport = TcpPort, sport = serverPort)#pkt[TCP] #uses separate tables to gather address info
                    #newPvtTcpPkt[TCP].dport = TcpPort
